...
const RoomSchema = new Schema({
    ...
    messages: Array //메세지 담는 배열
});
const Message = mongoose.model('Message', MessageSchema);
const Room = mongoose.model('Room', RoomSchema); //스키마 모델
class Chats{}

Chats.getUnreadCount = async function(room_id){
    try {
...
Chats.setRead = async function(room_id){
    try {
...
Chats.creatRoom = function(creator_id, participant_id){
    try {
          ...
          let result = new_room.save(); //save()는 생성된 객체 그대로 리턴
          return result;
    }
    catch(err) {
        console.log(err);
...
Chats.saveMessage = async function(s_id, r_id, con, room_id){
    try {
        ...
        await Room.update( //Room 컬렉션에 message 추가
            { _id: room_id },
            { $push: { messages: insertNew }}
        );
        return insertNew;
    }
    catch(err) {
