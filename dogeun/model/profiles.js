...
Profile.makeThumb = function(name, path){
...
Profile.editProfile = async function(req){
    try { 
         ...
         let original_url = profile[0].dataValues.profile_image;
         if(original_url){ //원래 프로필에 이미지가 있었으면
        ...
        if(req.file){
            ...
            await this.uploadThumbToS3(thumbnail_name, thumbnail_path);            
